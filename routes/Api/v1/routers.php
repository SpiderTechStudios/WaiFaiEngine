...
Route::middleware(['auth:sanctum', 'active.user', 'company.context', 'company.required'])->group(function () {
    Route::post('/device-setup', [DeviceSetupController::class, 'store'])->middleware('permission:'.Permissions::DEVICE_SETUP_CREATE);
    Route::patch('/device-setup', [DeviceSetupController::class, 'update'])->middleware('permission:'.Permissions::DEVICE_SETUP_UPDATE);
});
