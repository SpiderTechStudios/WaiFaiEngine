...
class WiFiDogTest extends TestCase
{
    ...
    public function test_public_prefix_alias_serves_wifidog_endpoints(): void
    {
        config([
            'captive.portal_url' => 'https://waifai.test/connect',
            'captive.portal_base_url' => 'https://waifai.test',
        ...
        $location = $this->get('/public/api/wifidog/login?gw_id=323&ip=192.168.0.35&mac=AA:BB:CC:DD:EE:FF')
            ->assertRedirect()
            ->headers->get('Location');
        ...
        $this->get('/public/api/wifidog/portal?gw_id=323&ip=192.168.0.35&mac=AA:BB:CC:DD:EE:FF')
            ->assertRedirect();

        $this->get('/public/api/wifidog/ping?gw_id=323&sys_uptime=1&sys_memfree=1&sys_load=0.1&wifidog_uptime=1')
            ->assertOk()
            ->assertSee('Pong', false);
    }

    public function test_portal_denied_message_does_not_loop_authenticated_client(): void
    {
        config([
            'captive.portal_url' => 'https://waifai.test/connect',
            'captive.portal_base_url' => 'https://waifai.test',
        ...
        $this->get('/api/wifidog/portal?gw_id=323&ip=192.168.0.35&mac=AA:BB:CC:DD:EE:FF&message=denied')
            ->assertRedirect()
            ->assertRedirectContains('/connect');
    }
